...
DATA_DIRECTORY = 'target_murine_cellline/'
TRAIN_SAMPLE_NUM = 4000
TAREGT_SAMPLE_NUM = 1000
BATCH_SIZE = 100
EPOCHS = 30
LEARNING_RATE = 0.00001
RESULT_DIRECTORY= 'Result_murine_cellline_scp_network/'
SAMPLE_SIZE=100
TARGET_TYPE="real"
...
EXPERT_PROPORTION=np.array([1/3,1/3,1/3])  
...
def mixup_fraction_test(celltype_num):
    ...
    return fracs
def mixup_cells(x, y, celltypes, sparse=False):
    ...
    fracs = mixup_fraction_test(no_avail_cts)
...
def train(train_data_x,train_data_y,target_data_x,target_data_y):
    ...
    encoder_da = nn.Sequential(EncoderBlockN(feature_num, 512, 0),
                                        EncoderBlockN(512, 256, 0.3))

    predictor_da = nn.Sequential(EncoderBlockN(256, 128, 0.2),
                                          nn.Linear(128, CELLTYPE_NUM),
                                          nn.Softmax(dim=1))

    discriminator_da = nn.Sequential(EncoderBlockN(256, 128, 0.2),
                                              nn.Linear(128, 1),
                                              nn.Sigmoid())
    optimizer_da1 = torch.optim.Adam([{'params': encoder_da.parameters()},
                                          {'params': predictor_da.parameters()},
                                          {'params': discriminator_da.parameters()}], lr=LEARNING_RATE)
    optimizer_da2 = torch.optim.Adam([{'params': encoder_da.parameters()},
                                          {'params': discriminator_da.parameters()}], lr=LEARNING_RATE)
    model_da = nn.ModuleList([])
    
    model_da.append(encoder_da)
    model_da.append(predictor_da)
    model_da.append(discriminator_da)
    model_da=model_da.cuda()
    criterion_da = nn.BCELoss()
    source_label = torch.ones(BATCH_SIZE).unsqueeze(1)
    target_label = torch.zeros(BATCH_SIZE).unsqueeze(1)
    ...
    for epoch in range(EPOCHS):
        ...
        for batch_idx, (source_x, source_y) in enumerate(train_train_loader):
            ...
            embedding_source = encoder_da(source_x)
            embedding_target = encoder_da(target_x)
            frac_pred = predictor_da(embedding_source)
            domain_pred_source = discriminator_da(embedding_source)
            domain_pred_target = discriminator_da(embedding_target)
            pred_loss = mse_loss(frac_pred, source_y)
        ...
        disc_loss_list.append(disc_loss_epoch)
        disc_loss_da_list.append(disc_loss_DA_epoch)   
    result_list=[]
    result_list.append(pred_loss_list)
    result_list.append(disc_loss_list)
    result_list.append(disc_loss_da_list)
    if TARGET_TYPE == "simulated":
        ...
        plot_loss_histories(result_list,['pred_loss', 'disc_loss','disc_loss_da'])
    preds, gt = None, None
    for batch_idx, (x, y) in enumerate(test_target_loader):
        model_da.eval()
        logits = predictor_da(encoder_da(x.cuda())).detach().cpu().numpy()
...
for good in range (20):
    ...
    g = torch.Generator()
    g.manual_seed(seed)
    TARGET_DATASET_NAME="murine_cellline_simulated_test_x_"+str(good)+".csv"
