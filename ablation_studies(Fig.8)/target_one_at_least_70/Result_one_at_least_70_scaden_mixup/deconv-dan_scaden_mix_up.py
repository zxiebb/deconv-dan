...
RESULT_DIRECTORY= 'Result_one_at_least_70_scaden_mixup/'
SAMPLE_SIZE=100
TARGET_TYPE="real"
...
def mixup_dataset(dataset, metadata, sample_num,labels):
    ...
    for _ in range(int(sample_num//2)):
        sample, label = mixup_cells(data_x, data_y, TYPE_LIST)
        sim_data_x.append(sample)
        sim_data_y.append(label)
    for _ in range(int(sample_num//2)):
        sample, label = mixup_cells(data_x, data_y, TYPE_LIST, sparse=True)
        sim_data_x.append(sample)
        sim_data_y.append(label)
