## Load in data
DIR<-"D:/Data/Methylation/450K_simulated/"
...
dataset<-0
for (dataset in 0:19){
props_path<-paste(DIR,"target_450K_simulated/450K_simulated_test_y_",sep="")
props_path<-paste(props_path,dataset,sep="")
props_path<-paste(props_path,".csv",sep="")
props<-read.csv(props_path, row.names = 1)
ref_path<-paste(DIR,"reference/reference_450K_mean_value.csv",sep="")
ref <- read.csv(ref_path, row.names = 1)
target_path<-paste(DIR,"target_450K_simulated/450K_simulated_test_x_",sep="")
