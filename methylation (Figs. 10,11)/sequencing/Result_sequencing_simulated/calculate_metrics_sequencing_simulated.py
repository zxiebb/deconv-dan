...
import random
DATASET = 'EPIC'
DATA_DIRECTORY = 'D:/Data/Methylation/sequencing_simulated/target_sequencing_simulated/'
RESULT_DIRECTORY= 'D:/Data/Methylation/sequencing_simulated/target_sequencing_simulated/Result_sequencing_simulated/'
...
for i in range(6):
    ...
    for good in range (20):
        prediction=pd.read_csv(RESULT_DIRECTORY+"target_predicted_fraction"+str(good)+".csv",index_col=0,header=0)
        ground_truth=pd.read_csv(DATA_DIRECTORY+"sequencing_test_y_"+str(good)+".csv",index_col=0,header=0)
        prediction=prediction[TYPE_LIST[i]]
        ground_truth=ground_truth[TYPE_LIST[i]]
        ...
        result_dataframe.to_csv(RESULT_DIRECTORY + "target_result_"+TYPE_LIST[i]+".csv")
CCC_LIST = []
RMSE_LIST = []
CORR_LIST = []
for good in range(20):
