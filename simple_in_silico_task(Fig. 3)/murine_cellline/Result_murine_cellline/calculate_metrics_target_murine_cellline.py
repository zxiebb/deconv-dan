...
RESULT_DIRECTORY= 'Result_murine_cellline/'
...
for i in range(3):
    ...
    for good in range (20):
        ...
        ground_truth=pd.read_csv(DATA_DIRECTORY+"murine_cellline_simulated_test_y_"+str(good)+".csv",index_col=0,header=0)
...
for i  in range(20):
    prediction = pd.read_csv(RESULT_DIRECTORY + "target_predicted_fraction" + str(good) + ".csv", index_col=0, header=0)
    ground_truth = pd.read_csv(DATA_DIRECTORY + "murine_cellline_simulated_test_y_" + str(good) + ".csv", index_col=0,
                               header=0)
